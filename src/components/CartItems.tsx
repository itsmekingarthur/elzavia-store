...
export default function CartItems() {
  ...
  if (submitted) {
    return (
      <div className="text-center py-12 md:py-20">
        ...
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4">تم تقديم الطلب بنجاح!</h2>
        <p className="text-gray-500 mb-8">سنقوم بالتواصل معك قريباً لتأكيد الطلب.</p>
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12 md:py-20">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 md:h-20 md:w-20 text-gray-300 mx-auto mb-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" />
        </svg>
        <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-4">سلتك فارغة</h2>
        <p className="text-gray-500 mb-8">أضف بعض المنتجات لبدء التسوق</p>
        <Link href="/shop" className="btn-primary">
          تصفح المنتجات
        </Link>
      </div>
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
      <div className="lg:col-span-2 space-y-3 md:space-y-4">
        {items.map((item) => {
          ...
          return (
            <div
              key={item.productId}
              className="flex items-center gap-3 md:gap-4 bg-white rounded-xl p-3 md:p-4 border border-gray-100"
            >
              <img
                src={product.images[0]}
                alt={product.name}
                className="w-14 h-14 md:w-20 md:h-20 object-contain bg-gray-50 rounded-lg flex-shrink-0"
              />
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-gray-900 text-sm md:text-base truncate">
                  {product.name}
                </h3>
                <p className="text-primary-600 font-bold text-sm md:text-base mt-0.5 md:mt-1">
                  {formatPrice(product.price)}
                </p>
                {item.dealLabel && item.dealDiscount && item.dealDiscount > 0 && (
                  <p className="text-green-600 text-[10px] md:text-xs font-medium mt-0.5">
              ...
              <div className="flex items-center gap-1 md:gap-2">
                <button
                  ...
                  className="w-7 h-7 md:w-8 md:h-8 rounded-full border border-gray-300 flex items-center justify-center hover:border-primary-500 hover:text-primary-600 transition-colors text-sm md:text-base"
                >
                  -
                </button>
      ...
      <div className="bg-white rounded-xl p-5 md:p-6 border border-gray-100 h-fit">
        <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-5 md:mb-6">ملخص الطلب</h3>

        <div className="space-y-3 mb-5 md:mb-6 pb-5 md:pb-6 border-b border-gray-100">
        ...
        <div className="mb-5 md:mb-6">
          <div className="flex gap-2">
            ...
            />
            <button onClick={applyCoupon} className="btn-primary !py-2 !px-4 text-sm whitespace-nowrap">
              تطبيق
            </button>
          </div>
          {couponError && (
        ...
        <div className="space-y-3 md:space-y-4">
          <h4 className="font-bold text-gray-900 text-sm md:text-base">معلومات التوصيل</h4>
          <input
            type="text"
            value={form.name}
          ...
          <button
            ...
            className="btn-primary w-full text-sm md:text-base disabled:opacity-50 disabled:cursor-not-allowed"
          >
